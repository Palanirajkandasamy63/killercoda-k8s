...
# >>> breakfix-03 mutation: the directory Deployment is scaled to 2 replicas that
#     share the SINGLE directory-data PVC (ReadWriteOnce). A podAntiAffinity forces
...
spec:
  ...
  template:
    metadata:
      labels: { app: directory, plane: app, tier: lab }
    spec:
      # MUTATED: force the two replicas onto different nodes, and allow one onto the
      # control-plane node, so the second consumer of the single RWO directory-data
      # volume lands on a node that cannot attach it. (Anti-affinity mechanics are M06;
      # here it only guarantees the two Pods split across the two nodes.)
      affinity:
        podAntiAffinity:
          requiredDuringSchedulingIgnoredDuringExecution:
            - labelSelector: { matchLabels: { app: directory } }
...
# leaves a workload Pending on storage, so `wait ... Available` never returns for
